...
public class SecureQRCode{
    ...
    public SecureQRCode(Context activity,String scanData, byte[] rawBytes) throws QRCodeException{
        mContext = activity;
        scannedAadharCard = new AadharCard();

        // 1. Convert Base10 to BigInt
       final BigInteger bigIntScanData = new BigInteger(scanData); //.substring(scanData.indexOf("uid=")+5,scanData.indexOf("uid=")+17)


        // 2. Convert BigInt to Byte Array
        final byte byteScanData[] = bigIntScanData.toByteArray(); //scanData.getBytes();
